# i18n — `apps/web` + `apps/admin` (vi / en)
...
| **Liên quan** | `docs/data-layer.md` (bắt buộc — feature này CÓ chạm backend), `docs/monorepo.md`, `docs/theme.md` (§7 so sánh cách lưu), `docs/blog.md` (§8) |
...
- **Mọi Server Component có dịch phải gọi `setRequestLocale(locale)`** trước khi dùng
  ...
  (`docs/blog.md`), mất static là mất luôn cache nginx.
