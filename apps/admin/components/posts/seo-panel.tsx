...
export function SeoPanel({
  ...
  const slugChangedAfterPublish =
    publishedSlug !== null && values.slug.trim() !== publishedSlug;

  return (
    <aside className="flex flex-col gap-5">
      <h2 className="text-sm font-medium uppercase tracking-wide opacity-60">SEO</h2>
      ...
      <div className="rounded-lg border border-black/10 p-3 dark:border-white/15">
        ...
        <p className="mt-1 text-base leading-snug text-blue-700 dark:text-blue-400">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        */}
        {slugChangedAfterPublish ? (
          ...
          >
            Bài này đang chạy ở <strong>/blogs/{publishedSlug}</strong>. Đổi slug
            thì URL cũ sẽ được chuyển hướng 301 sang slug mới — link cũ vẫn dùng
            được, nhưng nên có lý do để đổi.
          </p>
        ) : null}
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs opacity-60">
          Bỏ trống là đúng trong hầu hết trường hợp — canonical mặc định trỏ về
          chính bài này.
        </p>
      </div>
      ...
      <label className="flex items-start gap-2 text-sm">
        ...
        <span>
          ...
          <span className="block text-xs opacity-60">
            Bài vẫn công khai và vẫn có trong sitemap — chỉ là bảo công cụ tìm
            kiếm bỏ qua.
          </span>
        </span>
      </label>
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-black/10 dark:border-white/15">
      {image ? (
        ...
        <div className="flex aspect-[1200/630] w-full items-center justify-center bg-black/5 text-xs opacity-50 dark:bg-white/5">
          Chưa có ảnh — thẻ chia sẻ sẽ chỉ có chữ
        </div>
      )}
      <div className="border-t border-black/10 p-2.5 dark:border-white/15">
        <p className="truncate text-sm font-medium">{title}</p>
        <p className="mt-0.5 line-clamp-2 text-xs opacity-70">{description}</p>
      </div>
    </div>
  );
}
