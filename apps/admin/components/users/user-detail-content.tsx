...
export function UserDetailContent({ userId }: { userId: string }) {
  ...
  if (user.isPending) {
    return (
      <main className="w-full max-w-3xl space-y-4 p-6" aria-busy="true">
  ...
  return (
    <main className="w-full max-w-3xl p-6">
      <div className="flex items-center gap-3">
        ...
        <Badge
          tone={data.role === "admin" ? "info" : "neutral"}
          className="ml-auto"
        >
          {data.role}
        </Badge>
      ...
      <dl className="mt-6 grid grid-cols-[12rem_1fr] gap-y-3 text-sm">
        <dt className="opacity-60">Email</dt>
        <dd className="flex items-center gap-2">
          {data.email}
          {data.emailVerifiedAt ? (
            <Badge tone="success">
              Verify {formatDate(data.emailVerifiedAt)}
            </Badge>
          ) : (
        ...
        <dd className="font-mono text-xs opacity-70">{data.id}</dd>
        ...
        <dd>
          {data.usernameChangedAt
            ? formatDateTime(data.usernameChangedAt)
            : "Chưa từng đổi"}
        </dd>
      ...
      <p className="mt-6 rounded-md border border-black/10 p-3 text-xs opacity-60 dark:border-white/15">
        Chưa có hành động quản trị nào cho tài khoản này — khoá, đổi vai trò và
        thu hồi phiên đều cần endpoint chưa có ở backend (xem
